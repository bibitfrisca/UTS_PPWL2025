...
@section('content')
    <div class="container">
        <h5 style="color:#fff; background-color: #4B0D74; border: 2px solid;  padding: 10px; border-radius: 5px;">Add Stock In</h5>
        <form action="{{ route('stockins.store') }}" method="POST">
            @csrf
            <div class="form-group mb-3">
                <label for="stockin_code" style="color: #F05AE0;">Stock In Code</label>
                <input type="text" name="stockin_code" id="stockin_code" class="form-control" required>
            </div>
            <div class="form-group mb-3">
