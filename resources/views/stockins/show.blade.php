...
@section('content')
<div class="container">
    <h1 style="color: #4B0D74;">Stock In Details</h1>
    <div class="card" style="background-color: #D529B4; color: #FFFFFF; padding: 20px; border-radius: 10px;">
        <h3 style="color: #FFFFFF;">Stock In Code: {{ $stockin->stockin_code }}</h3>
        <p><strong>Date:</strong> {{ $stockin->date }}</p>
        <p><strong>Handled By:</strong> {{ $stockin->user?->name ?? 'Unknown' }}</p>
    </div>

    <h3 style="color: #4B0D74;" class="mt-4">Products in this Stock In</h3>
    ...
    <a href="{{ route('stockins.index') }}" class="btn btn-secondary" style="background-color: #8813A8; border-color: #8813A8;">Back to List</a>
</div>
@endsection
